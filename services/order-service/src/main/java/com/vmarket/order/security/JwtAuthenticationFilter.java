package com.vmarket.order.security;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
	 ...
	 *
	 * <p>Đã đối chiếu với token thật của PBL6-43 ({@code com.vmarket.auth.security.JwtService}):
	 * claim {@code roles} là mảng chuỗi {@code ["BUYER", "ADMIN"]} khớp {@code RoleName}.
	 */
	private static final String ROLES_CLAIM = "roles";

	private final SecretKey key;

	public JwtAuthenticationFilter(OrderJwtProperties properties) {
	...
	private void authenticate(String token) {
		try {
			// Cùng quy tắc với gateway: đúng chữ ký, đúng issuer, chưa hết hạn (lệch đồng
			// hồ tối đa 30 giây). Service vẫn tự verify dù gateway đã làm (NFR-SEC-03):
			// cổng 8086 publish ra host nên request có thể không đi qua gateway.
